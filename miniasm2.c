...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    if (strcmp(token, "exit") == 0) {
    ...
    } else if (strcmp(token, "geq_u64_o_c") == 0) {
      char *src1 = read_token(in);
      skip_ws(in);
      char *src2 = read_token(in);
      emit1(GEQ_U64_O_C, out, &prog_p);
      ...
      emit2(read_int2(src1), out, &prog_p);
    } else if (strcmp(token, "add_f64_s_o") == 0) {
      char *dest = read_token(in);
      emit1(ADD_F64_S_O, out, &prog_p);
      emit2(read_int2(dest), out, &prog_p);
    } else if (strcmp(token, "add_u64_c_o") == 0) {
      char *src = read_token(in);
      skip_ws(in);
      char *dest = read_token(in);
      emit1(ADD_U64_C_O, out, &prog_p);
      emit2(read_int2(dest), out, &prog_p);
      emit8(read_int8(src), out, &prog_p);
    } else if (strcmp(token, "cjump") == 0) {
      char *arg = read_token(in);
      size_t addr = get_or_add_label(&labels, arg, prog_p + 1);
      emit1(CJUMP, out, &prog_p);
